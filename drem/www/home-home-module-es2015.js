...
let HomePage = class HomePage {
    // user= {)
    constructor(rest, toastController, socket, formBuilder, http) {
        this.rest = rest;
    ...
    TakeCountry() {
        this.http
            ...
            console.log(data);
            this.pays = data;
            this.lepays = this.pays[7];
            this.countryflag = this.pays[7]["flag"];
        });
    }
    takecountryflag() {
    ...
    save() {
        ...
        if (!this.slideOneForm.valid) {
            ...
            this.rest.Adduser(this.slideOneForm.value).subscribe(data => {
                // console.log(data);
    ...
    logForm() {
        ...
        this.rest.Adduser(this.user).subscribe(data => {
            console.log(data);
            // this.user = {};
            this.Takeuser();
            this.presentToaste();
    ...
    log2Form() {
        ...
        this.rest.Addentreprise(this.entreprise).subscribe(data => {
            console.log(data);
        }, (error) => {
            console.log(error);
        });
    ...
    Deleteuser(c, i) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            yield elements[i].nativeElement.classList.remove("fadeInLeft");
            yield elements[i].nativeElement.classList.add(`delay-${t}s`);
            yield elements[i].nativeElement.classList.add("bounceOutUp");
            this.rest.Removeuser(c._id).subscribe(data => {
                console.log(data);
                setTimeout(() => {
                    this.removeAnimation();
